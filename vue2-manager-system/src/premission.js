...
/*
    Vue Router中的前置钩子函数beforeEach(to, from, next) 当进行路由跳转之前，进行判断是否已经登录过，登录过则允许访问非登录页面，否则，回到登录页面
    to：即将要进入的目标路由对象
    from：即将要离开的路由对象
...
router.beforeEach((to, from, next) => {
    ...
    if (!user) {
        ...
        if (to.path === '/login') { // 请求路由是/login，则去目标路由
            next()
        } else if (to.path === '/register') { // 请求路由是/register，则去register
            next()
        } else {
